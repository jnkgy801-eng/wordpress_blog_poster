...
# ----------------------------------------------------------------
# 🐦 WordPress投稿と同時にXへも紹介ポストを行う設定（任意・Buffer経由）
#    AUTO_POST_TO_X=true かつ WP_POST_STATUS=publish の場合のみ動作する。
#    （下書き/承認待ちの場合、まだ公開されていないページをXでリンクしてしまうため
#      安全のため意図的に投稿しない）
# ----------------------------------------------------------------
...
if AUTO_POST_TO_X and WP_POST_STATUS != 'publish':
    print('⚠️ AUTO_POST_TO_X=true ですが WP_POST_STATUS が publish ではないため、'
          'まだ公開されていないページをXでリンクしてしまうのを避けるため、X投稿は行いません。')
    AUTO_POST_TO_X = False

if AUTO_POST_TO_X and (not BUFFER_API_KEY or not BUFFER_CHANNEL_ID):
    print('⚠️ AUTO_POST_TO_X=true ですが BUFFER_API_KEY / BUFFER_CHANNEL_ID が未設定のため、X投稿は行いません。')
    AUTO_POST_TO_X = False

if AUTO_POST_TO_X:
    print('🐦 WordPress投稿と同時にX（Buffer経由）へも紹介ポストします。')
...
# ================================================================
# 🐦 X（Buffer経由）投稿：WordPress公開と同時に紹介ポストする
# ================================================================
...
def _x_char_weight(ch):
    # 半角英数記号=1、日本語・絵文字等=2（Xの重み付き文字数カウントの簡易再現）
    return 1 if ord(ch) < 0x1100 else 2
...
def build_x_catchcopy(product: dict) -> str:
    """シリーズ名・ジャンル・メーカー名から簡易的な一言キャッチコピーを組み立てる（AI不使用）。"""
    import random
    parts = []
    if product.get('series'):
        parts.append(f"『{product['series']}』シリーズ")
    if product.get('genres'):
        parts.append('×'.join(product['genres'][:2]))
    if not parts and product.get('maker'):
        parts.append(f"{product['maker']}の新作")
    if not parts:
        return ''
    templates = ["{info}が気になる一本", "{info}をチェック", "{info}な作品です"]
    return random.choice(templates).format(info='　'.join(parts[:2]))


def build_x_post_text(product: dict, wp_link: str, char_limit: int = 280) -> str:
    """WordPress記事のリンクを紹介するX投稿文を組み立てる（作品名・キャッチコピー・
    金額・出演者・WP記事URL・ハッシュタグ）。文字数超過時は優先度の低い順に削る。"""
    ...
    def assemble(title_limit, actor_limit, genre_limit, include_catchcopy):
        lines = [title_line(title_limit)]
        if include_catchcopy and catchcopy:
            lines.append(f"✏️ {catchcopy}")
        if price:
            lines.append(f"💰 {price}")
        actors = product.get('actors', [])[:actor_limit] if actor_limit else []
        act_tags = actor_tags(actors)
        if act_tags:
            lines.append(f"👤 {act_tags}")

        genres = product.get('genres', [])[:genre_limit] if genre_limit else []
        g_tags = genre_tags(genres)
        tag_line = '　'.join(t for t in [g_tags, base_tag] if t)

        return '\n\n'.join(['\n'.join(lines), wp_link, tag_line])

    title_limit, actor_limit, genre_limit, include_catchcopy = 35, 3, 3, True
    text = assemble(title_limit, actor_limit, genre_limit, include_catchcopy)
...
def buffer_graphql_request(query, variables=None, timeout=30):
    ...
    if data.get('errors'):
        return None, '; '.join(err.get('message', str(err)) for err in data['errors'])
    return data.get('data'), None


def post_to_x_via_buffer(product: dict, wp_link: str) -> bool:
    """WordPress公開と同時に、その記事URLを紹介するポストをBuffer経由でXへ即時投稿する。"""
    text = build_x_post_text(product, wp_link)
    input_obj = {
        'text': text,
        'channelId': BUFFER_CHANNEL_ID,
        'schedulingType': 'automatic',
        'mode': 'customScheduled',
        # WordPress公開の数分後に投稿されるよう、少しだけ間隔を空ける
        'dueAt': (datetime.datetime.utcnow() + datetime.timedelta(minutes=2)).strftime('%Y-%m-%dT%H:%M:%S.000Z'),
        'metadata': {'twitter': {'thread': [{'text': text}]}},
    }
    ...
    if result.get('message'):
        print(f'    ⚠️ X投稿失敗: {result["message"]}')
        return False
    if result.get('post'):
        print(f'    🐦 X投稿を予約しました（{result["post"].get("dueAt", "")}）')
        return True
    print('    ⚠️ X投稿失敗: 不明なエラー')
    return False


def post_draft_to_wordpress(article: dict):
        ...
        if resp.status_code in (200, 201):
            ...
            if actual_status != WP_POST_STATUS:
                print(f"    ⚠️ 指定したステータス（{WP_POST_STATUS}）と異なる値が返りました"
                      f"（status={actual_status}）。念のため内容をご確認ください: {link}")
            print(f"    ✅ {actual_status}として投稿成功: {article['title'][:40]}")
            return actual_status == 'publish', link
        else:

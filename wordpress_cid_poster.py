...
def build_article(product: dict) -> dict:
    ...
    if product.get('maker'):
        meta_line_html = (
            ...
            f'サークル: {escape(product["maker"])}</div>'
        )
    ...
    overview_section_html = (
        f'<h2 style="margin:14px 0 8px;font-size:17px;">『{escape(product["title"])}』はどんな作品？</h2>'
    ...
    if product.get('genres'):
        genre_section_html = (
            '<h2 style="margin:14px 0 8px;font-size:17px;">ジャンル・見どころ</h2>'
            f'{genre_badges_html}'
        )
    ...
    if product.get('price'):
        price_section_html = (
            '<h2 style="margin:14px 0 8px;font-size:17px;">価格・購入方法</h2>'
    ...
    cta_html = (
        ...
        f'▶「{escape(product["title"])}」の作品ページを見る</a></div>'
    )
    ...
    if product.get('review_avg') and (product.get('review_count') or 0) >= 50:
        section_order = [overview_section_html, star_html, meta_line_html,
                          genre_section_html, price_section_html,
                          points_section_html, gallery_html, video_html,
                          cta_html, internal_link_html, disclaimer_html]
    else:
        section_order = [overview_section_html, meta_line_html, star_html,
                          genre_section_html, price_section_html,
                          points_section_html, gallery_html, video_html,
                          cta_html, internal_link_html, disclaimer_html]

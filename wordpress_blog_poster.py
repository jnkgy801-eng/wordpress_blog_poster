...
⚠️ 安全設計についての重要な説明
--------------------------------------------------------------
- このスクリプトは記事を必ず「下書き（status=draft）」として投稿します。
  自動で「公開」まで行うモードは用意していません。
  最終的な公開判断は人間が行ってください。
  （旧はてな版はAtomPubの app:draft フラグの向きが誤っており、
    実際には自動公開されてしまうバグがありました。本スクリプトでは
    WordPress REST APIの status="draft" を明示的に指定し、
    投稿後のレスポンスでも status を検証してから成功と判定します）
- age_safety_filter.py により、タイトル・ジャンル・サークル名に
  未成年を想起させる語を含む作品は自動的に除外し、記事化しません。
  除外された件数と理由はログと outputs/skipped_*.txt に出力します。
  ⚠️ このフィルターのキーワードリストは初期状態では非常に少なく
  設定されています。運用前に必ず age_safety_filter.py の
  EXCLUDE_KEYWORDS を大幅に拡充してください。
- WordPress側の設定で年齢確認モーダル・注意書きの掲載、
  レンタルサーバーがアダルトコンテンツを許可しているかの確認、
  法令（景品表示法のPR表記等含む）を遵守する責任は運用者にあります。
--------------------------------------------------------------
"""
...
def _get_or_create_term(taxonomy: str, name: str, cache: dict):
    ...
    try:
        ...
        # 無ければ新規作成
        resp = requests.post(
            endpoint, json={'name': name}, auth=_wp_auth(), timeout=15,
        )
...
def post_draft_to_wordpress(article: dict) -> bool:
    ...
    # カテゴリーはWP側で「FANZA同人」「PR」のようなサイト全体の分類として使い、
    # ジャンル名はタグとして登録する（カテゴリーが乱立するのを防ぐため）
    category_id = _get_or_create_term('categories', 'FANZA同人', _category_cache)
    tag_ids = []
    ...
    try:
        resp = requests.post(endpoint, json=payload, auth=_wp_auth(), timeout=20)
        if resp.status_code in (200, 201):
            result = resp.json()

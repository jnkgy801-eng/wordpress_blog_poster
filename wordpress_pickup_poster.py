# -*- coding: utf-8 -*-
"""
📝 FANZA同人/動画 → WordPress「ピックアップ」記事自動生成・投稿ツール
--------------------------------------------------------------
wordpress_blog_poster.py をベースに、人気ランキング上位の作品を
「ピックアップ」カテゴリーへ1作品=1記事の形式で投稿する専用スクリプト。
（wordpress_blog_poster.py 本体には手を入れず、こちらは完全に別ファイルとして運用する）
--------------------------------------------------------------
"""
...
# 投稿先カテゴリー名の上書き。未設定なら従来どおり「同人」/「動画」を使うが、
# このスクリプト（ピックアップ専用）では基本的に「ピックアップ」を指定して使う。
CATEGORY_LABEL_OVERRIDE = os.environ.get('CATEGORY_LABEL_OVERRIDE', 'ピックアップ').strip()
if CATEGORY_LABEL_OVERRIDE:
    print(f'📌 投稿先カテゴリー: 「{CATEGORY_LABEL_OVERRIDE}」（CATEGORY_LABEL_OVERRIDEにより上書き）')
...
def build_article(product: dict, recent_openings: list = None) -> dict:
    ...
    internal_link_html = ''
    if WP_URL:
        cat_label = CATEGORY_LABEL_OVERRIDE or ('同人' if CONTENT_TYPE == 'doujin' else '動画')
        internal_link_html = (
    ...
    # このスクリプトではCATEGORY_LABEL_OVERRIDE（デフォルト「ピックアップ」）を優先する。
    display_category_label = CATEGORY_LABEL_OVERRIDE or ('同人' if CONTENT_TYPE == 'doujin' else '動画')

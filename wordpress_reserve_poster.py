# -*- coding: utf-8 -*-
"""
📝 FANZA予約作品 → WordPress 記事自動生成・投稿ツール
--------------------------------------------------------------
DMMアフィリエイトAPI(v3)から「発売日/配信日が未来」の予約作品を取得し、
作品ごとに記事（1作品=1記事）を生成して「予約作品」カテゴリーへ投稿する。
発売日は投稿のカスタムフィールド（_onavi_release_date）に保存し、
WordPress側で発売日ごとのタブ表示に利用する。

wordpress_blog_poster.py（発売済み作品用）とは別運用の、姉妹スクリプト。
投稿履歴ファイルも別（POSTED_HISTORY_FILE）にしているため、
同一作品が「予約作品」として投稿された後、発売日を迎えて
wordpress_blog_poster.py 側でも改めて投稿されることがある
（＝予約時と発売後で記事が2本になる）。仕様として許容する場合は
そのままでよいが、1本にまとめたい場合は別途「発売日を迎えた予約投稿の
カテゴリーを差し替える」バッチ処理が必要（未実装）。
--------------------------------------------------------------
"""
...
def build_article(product: dict) -> dict:
    ...
    # カテゴリーは常に「予約作品」の1つだけを使う（doujin/avの区別はタグ側のジャンルで表現する）。
    display_category_label = '予約作品'

    # 発売日/配信日（YYYY-MM-DD形式）。WordPress側の日付タブ表示で使用する。
    release_date = ''
    _d = _parse_dmm_date(product.get('date', ''))
    if _d:
        release_date = _d.strftime('%Y-%m-%d')
...
def post_draft_to_wordpress(article: dict) -> bool:
    ...
    # 発売日/配信日（YYYY-MM-DD）。WordPress側の予約作品ページで
    # 日付タブごとに記事をグループ化する際のキーとして使用する。
    # ※ WordPress側で '_onavi_release_date' を register_post_meta() 等で
    #   REST APIに公開しておく必要がある。
    if article.get('release_date'):
        meta['_onavi_release_date'] = article['release_date']
...
def main():
    ...
    today = now_jst.date()
    window_end = today + datetime.timedelta(days=RESERVE_WINDOW_DAYS)
    # 比較用に、実行時刻（JST）をtzなしdatetimeにしておく（DMMのdateフィールドはtz情報を持たないため）
    now_naive = now_jst.replace(tzinfo=None)
    # DMM API側で発売日/配信日の範囲を絞り込み、「今より未来」〜「RESERVE_WINDOW_DAYS日後まで」の
    # 予約作品のみを取得対象にする（発売済み・遠い未来すぎる作品は最初から除外）。
    gte_date_str = now_naive.strftime('%Y-%m-%dT%H:%M:%S')
    lte_date_str = datetime.datetime.combine(window_end, datetime.time.max).strftime('%Y-%m-%dT%H:%M:%S')
    ...
    print(f'📅 {len(raw_items)}件を取得しました。'
          f'（対象期間: 実行日時（{now_naive}）より未来 〜 {window_end} の発売日/配信日のみ投稿対象）')

    # 発売日が近い順（昇順）に並べ替える。直近の予約作品から優先的に記事化するため。
    raw_items_with_date = []
    for item in raw_items:
        d = _parse_dmm_date(item.get('date', ''))
        raw_items_with_date.append((d or datetime.datetime.max, item))
    raw_items_with_date.sort(key=lambda x: x[0])
    raw_items = [item for _, item in raw_items_with_date]
    ...
    for item in raw_items:
        ...
        # 発売日/配信日が「実行日時（now）より未来」かつ「今日から未来RESERVE_WINDOW_DAYS日以内」の
        # 予約作品のみを対象にする。
        # ・実行日時より過去（＝すでに発売済み）は自動的に対象外になる
        # ・window_endより先の作品も対象外になる
        product_date = _parse_dmm_date(product.get('date', ''))
        if (
            product_date is None
            or product_date <= now_naive
            or product_date.date() > window_end
        ):
    ...
    if out_of_window_skipped:
        print(f'   📅 対象期間（{today}〜{window_end}）外のためスキップ（例）:')
